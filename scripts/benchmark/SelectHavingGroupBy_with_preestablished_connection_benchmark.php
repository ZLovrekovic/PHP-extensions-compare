...
$stmt3->bind_param("si", $productLine, $minAvg);
...
$pdo2->setAttribute(PDO::ATTR_EMULATE_PREPARES, true); // true -uključuje emulaciju - koristi emulirane prepared statements

$stmt2 = $pdo2->prepare($query3);
$stmt2->bindParam(':productLine', $productLine, PDO::PARAM_STR);
$stmt2->bindParam(':minAvg', $minAvg, PDO::PARAM_INT);

benchmark(function($stmt) {
    $stmt->execute();
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {}
	$stmt->closeCursor();
}, "pdo_prepared_static_e", 200, $stmt2);
...
$pdo3->setAttribute(PDO::ATTR_EMULATE_PREPARES, false); // isključuje emulaciju - koristi prave prepared statements
$stmt3 = $pdo3->prepare($query3);
$stmt3->bindParam(':productLine', $productLine, PDO::PARAM_STR);
$stmt3->bindParam(':minAvg', $minAvg, PDO::PARAM_INT);

benchmark(function($stmt) {
    $stmt->execute();
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {}
	$stmt->closeCursor();
}, "pdo_prepared_static_n", 200, $stmt3);
